...
export default function PopularPostsPage() {
  ...
  useEffect(() => {
    const fetchPopularPosts = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8000/api/posts/popular/');
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl font-semibold text-gray-900 mb-8 text-center flex items-center justify-center gap-2">
          <Flame size={24} className="text-red-500" /> Popular Posts
        </h1>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {posts.length === 0 ? (
              <p className="text-center text-gray-500">No popular posts found.</p>
            ) : (
              posts.map((post) => (
                <div
                  key={post.id}
